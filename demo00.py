...
def get_word_num_map(labels):
    ...
    return words, words_size, word_num_map


# step01.将音频数据转为时间序列（列）和MFCC（行）的矩阵，将对应的译文转成字向量
...
def audiofile_to_input_vector(audio_filename, numcep, numcontext):
    ...
    train_inputs = np.float32(np.zeros(shape=(orig_inputs.shape[0], numcep * (2 * numcontext + 1))))
    empty_mfcc = np.float32(np.zeros(shape=(numcep)))
...
# step02.实现next_batch函数，获取下一batch的训练数据
def next_batch(wav_files, labels, start_idx=0, batch_size=1):
    ...
    # 01.将音频文件转成要训练的数据
    (source, audio_len, target, transcript_len) = get_audio_and_transcriptch(None, wav_files, n_input, n_context, word_num_map, txt_labels)
...
def pad_sequences(sequences, maxlen=None, dtype=np.float32, padding='post', truncating='post', value=0.):
    ...
    for idx, s in enumerate(sequences):
        ...
        if truncating == 'pre':
            ...
            trunc = s[:maxlen]  # 疑问：补0是否成功，debug吧？
        else:
...
def BiRNN_model(batch_x, seq_length, n_input, n_context, n_character, keep_dropout):
    ...
    with tf.name_scope('fc1'):
        b1 = variable_on_cpus('b1', [n_hidden_1], tf.random_normal_initializer(stddev=b_stddev))
        h1 = variable_on_cpus('h1', [n_input + 2 * n_input * n_context, n_hidden_1],
    ...
    with tf.name_scope('fc2'):
        b2 = variable_on_cpus('b2', [n_hidden_2], tf.random_normal_initializer(stddev=b_stddev))
        h2 = variable_on_cpus('h2', [n_hidden_1, n_hidden_2], tf.random_normal_initializer(stddev=h_stddev))
        layer_2 = tf.minimum(tf.nn.relu(tf.add(tf.matmul(layer_1, h2), b2)), relu_clip)
        layer_2 = tf.nn.dropout(layer_2, keep_dropout)

    # 3rd layer
    with tf.name_scope('fc3'):
        b3 = variable_on_cpus('b3', [n_hidden_3], tf.random_normal_initializer(stddev=b_stddev))
        h3 = variable_on_cpus('h3', [n_hidden_2, n_hidden_3], tf.random_normal_initializer(stddev=h_stddev))
    ...
    with tf.name_scope('lstm'):
        ...
        outputs, output_states = tf.nn.bidirectional_dynamic_rnn(cell_fw=lstm_fw_cell, cell_bw=lstm_bw_cell, inputs=layer_3,
                                                                 dtype=tf.float32, time_major=True, sequence_length=seq_length)
    ...
    with tf.name_scope('fc5'):
        b5 = variable_on_cpus('b5', [n_hidden_5], tf.random_normal_initializer(stddev=b_stddev))
        h5 = variable_on_cpus('h5', [(2 * n_cell_dim), n_hidden_5], tf.random_normal_initializer(stddev=h_stddev))
        layer_5 = tf.minimum(tf.nn.relu(tf.add(tf.matmul(outputs, h5), b5)), relu_clip)
        layer_5 = tf.nn.dropout(layer_5, keep_dropout)

    with tf.name_scope('fc6'):
        # 全连接层用于softmax分类
        b6 = variable_on_cpus('b6', [n_character], tf.random_normal_initializer(stddev=b_stddev))
        h6 = variable_on_cpus('h6', [n_hidden_5, n_character], tf.random_normal_initializer(stddev=h_stddev))
...
def variable_on_cpus(name, shape, initializer):
    with tf.device('/cpu:0'):
        var = tf.get_variable(name=name, shape=shape, initializer=initializer)
    return var

# 测试阶段：获取要训练的音频文件路径和对应的译文
...
def run_model():
    ...
    # 优化器:a.自适应学习率优化算法主要有：AdaGrad算法，RMSProp算法，Adam算法以及AdaDelta算法;
    # b.tf.train.GradientDescentOptimizer, 梯度下降算法的Optimizer;
    # Adam通常被认为对超参数的选择相当鲁棒
    learning_rate = 0.001
    optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(avg_loss)

    # CTC decoder:使用ctc_beam_search_decoder函数对预测结果进行解码，它返回值decoded是一个只有一个元素的数组;
    with tf.name_scope("decode"):
        decoded, log_prob = ctc_ops.ctc_beam_search_decoder(logits, seq_length, merge_repeated=False)

    # 算编辑距离:所以，使用edit_distance函数计算编辑距离时，应该传入的是decoded[0]。最后，对编辑距离取均值，求平均错误率;
...
if __name__ == '__main__':
    ...
    wav_path = 'CSLT/thchs30-openslr/data_thchs30/train001'
    tran_path = 'CSLT/thchs30-openslr/data_thchs30/data001'
    wav_files, tran_texts = get_wav_files_and_tran_texts(wav_path, tran_path)
    txt_files = tran_texts
    labels = tran_texts
    # 梅尔倒谱系数的个数
    n_input = 26
    ...
    # batch大小
    batch_size = 5
    words, words_size, word_num_map = get_word_num_map(labels)
    # step01.将音频数据转为时间序列（列）和MFCC（行）的矩阵，将对应的译文转成字向量
    # audio, audio_len, transcript, transcript_len = get_audio_and_transcriptch(txt_files, wav_files, n_input, n_context, word_num_map, txt_labels=None)
    # file = pd.DataFrame(transcript)
    # file.to_csv("data.csv")

    # step02.获取下一batch的训练数据：得到某一批次的音频数据(已补齐)以及已稀疏的翻译文本内容
    start_idx, source, source_lengths, sparse_lab = next_batch(wav_files, labels, start_idx=0, batch_size=5)
    # 测试：将字向量转成文字
    # results = sparse_tuple_to_texts_ch(sparse_lab, words)
    # print('results:\n', results)
    # print(u'音频数据转成MFCC特征向量矩阵：', source)
    # print(u'翻译文本转成稀疏矩阵：', sparse_lab)
    ...
    end_time = time.clock()
    print("===================")
    print('run_time:%fs' % round(end_time - start_time, 4))